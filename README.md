...
> 💡 **No API key? Use OpenCode Zen free models.** Set `LLM_MODEL=zen/deepseek-v4-flash-free` and you can skip `LLM_API_KEY` entirely — only `LLM_MODEL` and `GATEWAY_TOKEN` are required. See [OpenCode Zen — Free Models](#-opencode-zen--free-models-no-api-key).
...
> [!WARNING]
> Hugging Face's free tier puts Spaces to **sleep after ~48 hours of inactivity** — this is intended behavior. **Automated keep-alive pinging is disabled by default** in HuggingClaw because it can put your Hugging Face account at risk of suspension. Enable it only at your own risk by setting `CLOUDFLARE_KEEPALIVE_ENABLED=true` together with `CLOUDFLARE_WORKERS_TOKEN`.

To wake a sleeping Space, just open its URL again. For genuinely always-on operation, upgrade to a paid plan — see the [Spaces Overview](https://huggingface.co/docs/hub/en/spaces-overview).
...
| `CLOUDFLARE_KEEPALIVE_ENABLED` | `false` | Keep-alive pinging is OFF by default (account-suspension risk). Set to `true` only if you accept that risk |
...
## 🤖 OpenCode Zen — Free Models (no API key)

HuggingClaw can run entirely on **OpenCode Zen's free models** — no `LLM_API_KEY` needed. Set just two secrets:

- `LLM_MODEL` = `zen/deepseek-v4-flash-free` (or any other free model below)
- `GATEWAY_TOKEN` = your token

**How it works:** a small local relay (`zen-relay.mjs`) runs inside the Space and forwards OpenAI-compatible requests to `https://opencode.ai/zen`, stripping the `Authorization` header and setting the OpenCode CLI `User-Agent` — both of which Zen's free tier requires. OpenClaw registers `zen` as a local provider, so there is no API key, no probe traffic, and no pinging.

**Available free models** (switch anytime with `/model`):

| Model ref | Context |
| :--- | :--- |
| `zen/deepseek-v4-flash-free` | 256K *(default)* |
| `zen/mimo-v2.5-free` | 128K *(vision)* |
| `zen/big-pickle` | 128K |
| `zen/nemotron-3-ultra-free` | 128K |
| `zen/nemotron-3.5-lightning-free` | 128K |
| `zen/hy3-free` | 64K |
| `zen/laguna-s-2.1-free` | 64K |

**Vision:** image inputs are routed to `zen/mimo-v2.5-free` automatically whenever the active model can't accept images.

> [!WARNING]
> Free Zen models are temporary feedback programs — OpenCode may use free-tier prompts to improve the models, and NVIDIA-backed free endpoints are trial-use only. Do not send personal or confidential data. See [OpenCode Zen](https://opencode.ai/docs/zen/) for the current list and terms.

Optional variables: `ZEN_RELAY_PORT` (default `11435`), `ZEN_TARGET` (default `https://opencode.ai/zen`).
