...
# OpenCode Zen free-tier mode: `zen/*` models need no API key (local relay
# strips auth + sets the OpenCode CLI User-Agent for the free tier).
ZEN_FREE_ENABLED=false
case "$LLM_MODEL" in
  zen/*) ZEN_FREE_ENABLED=true ;;
esac
...
ERRORS=""
if [ -z "$LLM_API_KEY" ] && [ "$ZEN_FREE_ENABLED" != "true" ]; then
...
case "$LLM_PROVIDER" in
  ...
  # ── OpenCode Zen free (local relay) ──
  zen)                          : ;;  # no key needed; zen-relay handles auth + UA
...
# ── OpenCode Zen free-tier provider (via local relay) ──
# Registers `zen/*` as an OpenAI-compatible custom provider pointing at the
# local relay. The relay forwards to https://opencode.ai/zen, strips the
# Authorization header, and spoofs the OpenCode CLI User-Agent — both required
# by Zen's free models. No API key is ever sent.
ZEN_RELAY_PORT="$(trim_var "${ZEN_RELAY_PORT:-11435}")"
ZEN_MODELS_JSON=$(jq -n '[
  { id: "deepseek-v4-flash-free",        name: "DeepSeek V4 Flash (Free)",        contextWindow: 256000, maxTokens: 8192, input: ["text"] },
  { id: "mimo-v2.5-free",                name: "MiMo V2.5 (Free)",                contextWindow: 128000, maxTokens: 8192, input: ["text", "image"] },
  { id: "big-pickle",                    name: "Big Pickle (Free)",               contextWindow: 128000, maxTokens: 8192, input: ["text"] },
  { id: "nemotron-3-ultra-free",         name: "Nemotron 3 Ultra (Free)",         contextWindow: 128000, maxTokens: 8192, input: ["text"] },
  { id: "nemotron-3.5-lightning-free",   name: "Nemotron 3.5 Lightning (Free)",   contextWindow: 128000, maxTokens: 8192, input: ["text"] },
  { id: "hy3-free",                      name: "Hy3 (Free)",                      contextWindow: 64000,  maxTokens: 8192, input: ["text"] },
  { id: "laguna-s-2.1-free",             name: "Laguna S 2.1 (Free)",             contextWindow: 64000,  maxTokens: 8192, input: ["text"] }
]')

if [ "$ZEN_FREE_ENABLED" = "true" ]; then
  echo "Registering OpenCode Zen free provider (zen -> http://127.0.0.1:${ZEN_RELAY_PORT}/v1)..."
  CONFIG_JSON=$(jq \
    --arg provider "zen" \
    --arg baseUrl "http://127.0.0.1:${ZEN_RELAY_PORT}/v1" \
    --argjson models "$ZEN_MODELS_JSON" \
    '.models.mode = "merge"
     | .models.providers[$provider] = {
         "baseUrl": $baseUrl,
         "apiKey": "local",
     ...
     | .agents.defaults.imageModel = { "primary": "zen/mimo-v2.5-free" }' <<<"$CONFIG_JSON")
fi
...
# ── Start OpenCode Zen relay (free-tier models) ──
ZEN_RELAY_PID=""
start_zen_relay_once() {
  [ "$ZEN_FREE_ENABLED" = "true" ] || return 0
  if [ -n "${ZEN_RELAY_PID:-}" ] && kill -0 "$ZEN_RELAY_PID" 2>/dev/null; then
    return 0
  fi
  echo "Zen relay : starting on 127.0.0.1:${ZEN_RELAY_PORT:-11435}..."
  node /home/node/app/zen-relay.mjs >> /tmp/zen-relay.log 2>&1 &
  ZEN_RELAY_PID=$!
  echo "Zen relay : started (PID: $ZEN_RELAY_PID)"
}
start_zen_relay_once
...
while true; do
  ...
  # Check Zen relay process - restart if died unexpectedly
  if [ "$ZEN_FREE_ENABLED" = "true" ] && { [ -z "${ZEN_RELAY_PID:-}" ] || ! kill -0 "$ZEN_RELAY_PID" 2>/dev/null; }; then
    echo "Warning: Zen relay not running; restarting..."
    unset ZEN_RELAY_PID
    start_zen_relay_once
  fi
